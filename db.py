...
def init_db():
    with connect() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS repeat_questions (
                source_id TEXT PRIMARY KEY,
...
def save_repeat_question(source_id, reminder_id):
    with connect() as conn:
        conn.execute(
            """
            INSERT INTO repeat_questions(source_id, reminder_id, created_at)
            VALUES(?, ?, ?)
            ON CONFLICT(source_id) DO UPDATE SET
                reminder_id = excluded.reminder_id,
                created_at = excluded.created_at
            """,
            (source_id, reminder_id, now_iso()),
        )
...
def get_repeat_question(source_id):
    with connect() as conn:
        row = conn.execute(
            "SELECT reminder_id FROM repeat_questions WHERE source_id = ?",
            (source_id,),
        ).fetchone()
    return dict(row) if row else None
...
def clear_repeat_question(source_id):
    with connect() as conn:
        conn.execute("DELETE FROM repeat_questions WHERE source_id = ?", (source_id,))
...
def set_reminder_recurrence(source_id, reminder_id, recurrence):
    with connect() as conn:
        cursor = conn.execute(
            ...
            UPDATE reminders
            SET recurrence = ?
            WHERE source_id = ? AND id = ? AND status = 'pending'
            """,
            (recurrence, source_id, reminder_id),
        )
...
def complete_interval_reminders(source_id):
    with connect() as conn:
        cursor = conn.execute(
            """
            UPDATE reminders
            SET status = 'completed', sent_at = ?
            WHERE source_id = ?
              AND status = 'pending'
              AND recurrence LIKE 'interval:%'
            """,
            (now_iso(), source_id),
        )
    return cursor.rowcount
