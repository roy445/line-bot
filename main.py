...
def reminder_quick_reply():
    return QuickReply(
        items=[
            QuickReplyButton(action=MessageAction(label="設定提醒", text="設定提醒")),
...
def is_reminder_query(text):
    return bool(
        re.search(r"(我的提醒|查看提醒|提醒列表|有哪些提醒|有什麼提醒|今天還有什麼|待辦|還有什麼事)", text)
    )
...
def maybe_complete_draft(event, sid, text):
    ...
    db.save_pending_reminder(
        sid,
        draft["target_id"],
        draft["title"],
        remind_at.isoformat(timespec="seconds"),
        f"{draft['raw_text']} / {text}",
        recurrence,
    )
    ...
    reply_text(
        ...
        "這樣可以嗎？\n"
        f"內容：{pending['title']}\n"
        f"時間：{format_reminder_time(pending['remind_at'])} {format_recurrence(pending.get('recurrence'))}",
...
def handle_reminder_analysis(event, sid, target_id, text, analysis):
    ...
    if analysis["confidence"] >= 0.8:
        db.add_reminder(sid, target_id, title, remind_at, recurrence)
        reply_text(
            event.reply_token,
            "提醒設定好了。\n"
            f"內容：{title}\n"
...
def handle_text(event):
    ...
    if delete_command:
        if delete_command[0] == "range":
            count = db.delete_pending_reminder_range(sid, delete_command[1], delete_command[2])
        else:
            count = db.delete_pending_reminders(sid, delete_command[1])
        reply_text(
            event.reply_token,
            f"已取消 {count} 個提醒。" if count else "找不到符合的待提醒編號，輸入「我的提醒」看一下目前有哪些。",
        )
        return
    ...
    if is_memory_query(text):
        ...
        if not memories:
            reply_text(event.reply_token, "我目前還沒有記住你的特別資訊。")
        else:
            reply_text(event.reply_token, "我記得這些：\n" + "\n".join(f"- {item['content']}" for item in memories))
        return
